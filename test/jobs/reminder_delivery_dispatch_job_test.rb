...
class ReminderDeliveryDispatchJobTest < ActiveJob::TestCase
  test "enqueues due pending email deliveries for one account" do
    ...
    due = ReminderDelivery.create!(user: users(:one), source: setting.person, channel: "email", reminder_on: Date.new(2026, 8, 8), occurrence_on: Date.new(2026, 8, 8))
    future = ReminderDelivery.create!(user: users(:one), source: setting.person, channel: "email", reminder_on: Date.new(2026, 8, 9), occurrence_on: Date.new(2026, 8, 9))
    other_person = people(:bob)
    other_person.create_keep_in_touch_setting!(cadence: "weekly", enabled_on: Date.new(2026, 8, 1))
    other_user = ReminderDelivery.create!(user: users(:two), source: other_person, channel: "email", reminder_on: Date.new(2026, 8, 8), occurrence_on: Date.new(2026, 8, 8))
