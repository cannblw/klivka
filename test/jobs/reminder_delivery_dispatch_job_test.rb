...
class ReminderDeliveryDispatchJobTest < ActiveJob::TestCase
  test "enqueues due pending email deliveries for one account" do
    ...
    other_user = ReminderDelivery.create!(user: users(:two), source: friends(:bob).create_keep_in_touch_setting!(cadence: "weekly", enabled_on: Date.new(2026, 8, 1)), channel: "email", reminder_on: Date.new(2026, 8, 8), occurrence_on: Date.new(2026, 8, 8))
