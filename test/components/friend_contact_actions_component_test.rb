...
class FriendContactActionsComponentTest < ViewComponent::TestCase
  test "renders no actions without usable phone or email entries" do
    ...
    assert_no_selector "[aria-label='Quick contact actions']"
  end
  ...
  test "renders up to two actions per type and puts every value in the dialog" do
    ...
    assert_selector "[aria-label='Quick contact actions'] a[href^='tel:']", count: 2
    assert_selector "[aria-label='Quick contact actions'] a[href^='mailto:']", count: 2
    assert_selector "[aria-label='Call 555-1000']", text: "Mobile"
