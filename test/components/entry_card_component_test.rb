...
class EntryCardComponentTest < ViewComponent::TestCase
  test "renders a type-labeled sortable entry card" do
    ...
    render_inline(EntryCardComponent.new(entry: entry, person: entry.person))
    ...
    edit_path = Rails.application.routes.url_helpers.edit_person_entry_path(entry.person, entry)
  ...
  test "birthday card shows only month and day when the birth year is unknown" do
    ...
    render_inline EntryCardComponent.new(entry: birthday, person:)
  ...
  test "describes a same-day reminder naturally in Spanish" do
    entry = Entry::Date.create!(person: people(:ada), entry_date: Date.new(2026, 8, 11), label: "Aniversario")
  ...
  test "does not show the leap-day rule for a one-time reminder" do
    entry = Entry::Date.create!(person: people(:ada), entry_date: Date.new(2028, 2, 29))
