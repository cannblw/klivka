...
class AccountExportSerializerTest < ActiveSupport::TestCase
  ...
  test "keeps another account and internal state out of the export" do
    ...
    assert_empty export["account"].keys & %w[id password_digest confirmed_at reminders_scanned_through_on]
    ...
    assert export["categories"].none? { |category| category.key?("id") }
    assert export["contact_methods"].none? { |contact_method| contact_method.key?("id") }
    assert export["people"].none? { |person| person.key?("id") || person.key?("category_id") }
  end
  ...
  test "orders exported collections deterministically" do
    ...
    assert_equal [ person.name, later_person.name, earlier_person.name ], export["people"].pluck("name")
    assert_equal "phone", export["people"].first["entries"].last["type"]
    assert_equal user.categories.order(:id).pluck(:name), export["categories"].pluck("name")
    assert_equal user.contact_methods.order(:id).pluck(:name), export["contact_methods"].pluck("name")
  end
  ...
  def assert_entry_contract(entries, person)
    expected_keys = %w[type position content entry_date birthday_year_known created_at updated_at reminder]
    ...
    reminder = person.entries.find_by!(type: "Entry::Date").entry_reminder
    assert_equal({
      "lead_value" => 5,
      "lead_unit" => "days",
      "recurrence" => "yearly",
