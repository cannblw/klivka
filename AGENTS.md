...
- When adding or changing a product feature, revisit the demo reset in `DemoSeeder#reset!` and `DemoPersonaSeeder` to decide whether new user-owned data needs to be cleared and re-seeded as part of the reset. The persona seeder must always find the state it expects; any mismatch raises at reset time and leaves the demo broken.
