module AccountImport
  class Document
    ...
    def validate!
      ...
      invalid!(:unsupported_version) unless SUPPORTED_FORMAT_VERSIONS.include?(payload["format_version"])
    ...
    def source_email_address
      payload.dig("account", "email_address")
    end

    def summary
      people = payload.fetch("people")
      entries = people.sum { |person| person.fetch("entries").size }
      reminders = people.count { |person| person["keep_in_touch_setting"] } +
